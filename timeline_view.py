...
class TimelineView(QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        if self.is_dragging_clip:
            ...
            new_x = self.get_snapped_x(new_pos.x())
            new_y = round(new_pos.y() / self.track_height) * self.track_height
    ...
    def dragEnterEvent(self, e): e.accept() if e.mimeData().hasUrls() else super().dragEnterEvent(e)
    def dragMoveEvent(self, e): e.accept() if e.mimeData().hasUrls() else super().dragMoveEvent(e)
    ...
    def fit_to_view(self):
        ...
        end = max(i.start + i.duration for i in items)
        dur = end - start
