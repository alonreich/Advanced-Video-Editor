...
class MainWindow(QMainWindow):
    ...
    def on_media_pool_double_click(self, path):
        occupied_tracks = set()
        if self.timeline.timeline_view.scene:
            for item in self.timeline.timeline_view.scene.items():
        ...
        while track in occupied_tracks:
            track += 1
        start = 0.0
        self.asset_loader.handle_drop(path, track, start)
        try:
    ...
    def setup_ui(self):
        ...
        self.setStyleSheet("QMainWindow::separator { background-color: #CCCCCC; width: 7px; height: 7px; }")
        ...
        self.dock_pool.setFeatures(QDockWidget.DockWidgetMovable | QDockWidget.DockWidgetFloatable)
        self.dock_insp.setFeatures(QDockWidget.DockWidgetMovable | QDockWidget.DockWidgetFloatable)
